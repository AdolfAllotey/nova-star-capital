...
from social.utils.utils_volume import get_token_volume
...
# === Résultats Simulation de Trades Journaliers ===
with st.expander("📅 Résultats simulations journalières"):
    st.subheader("📊 Résumé des trades simulés")

    import os
    import pandas as pd
    import matplotlib.pyplot as plt

    report_path = "data/trades/trades_simulated_all.csv"
    ...
    if os.path.exists(report_path):
        ...
        fig, ax = plt.subplots(figsize=(10, 4))
        ax.plot(df.index, df["cumulative_profit"], linewidth=2)
        ax.set_title("📈 Cumul des profits simulés")
        ...
        st.download_button(
            ...
            file_name="trades_simulated_all.csv",
            mime="text/csv"
        )
    else:
        st.info("Aucune donnée de simulation trouvée.")
