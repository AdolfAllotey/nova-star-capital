# src/v2/api/routes/simulation.py
from fastapi import APIRouter
from pydantic import BaseModel
from typing import List, Optional, Any
import json
import datetime as dt
from pathlib import Path

from src.v2.utils.file_utils import get_data_dir

router = APIRouter(prefix="/simulation", tags=["simulation"])

DATA_ROOT = Path(get_data_dir())
OPEN_POS_PATH = DATA_ROOT / "trading" / "open_positions.json"


class OpenPosition(BaseModel):
    token: str
    qty: float
    entry: float
    pnl: float
    exit_reason: Optional[str] = None
    ts: Optional[str] = None


class OpenPositionsResp(BaseModel):
    updated_at: Optional[str] = None
    open_positions: List[OpenPosition] = []
    message: Optional[str] = None


def _read_json(path: Path) -> Any:
    try:
        with path.open("r", encoding="utf-8") as f:
            return json.load(f)
    except FileNotFoundError:
        return None
    except Exception:
        return None


@router.get("/open-positions", response_model=OpenPositionsResp)
def get_open_positions():
    raw = _read_json(OPEN_POS_PATH)
    if not isinstance(raw, list):
        if isinstance(raw, dict) and isinstance(raw.get("items"), list):
            raw = raw["items"]
        else:
            return OpenPositionsResp(
                updated_at=dt.datetime.utcnow().replace(microsecond=0).isoformat() + "Z",
                open_positions=[],
                message="Aucune position (trading/open_positions.json).",
            )

    out: List[OpenPosition] = []
    for it in raw:
        if not isinstance(it, dict):
            continue
        try:
            out.append(
                OpenPosition(
                    token=str(it.get("token") or it.get("symbol") or "").upper(),
                    qty=float(it.get("qty") or it.get("quantity") or it.get("remaining_size") or it.get("size") or 0),
                    entry=float(it.get("entry") or it.get("entry_price") or 0),
                    pnl=float(it.get("pnl") or it.get("pnl_eur") or it.get("realized_pnl") or 0),
                    exit_reason=it.get("exit_reason"),
                    ts=it.get("ts") or it.get("timestamp") or it.get("opened_at"),
                )
            )
        except Exception:
            continue

    return OpenPositionsResp(
        updated_at=dt.datetime.utcnow().replace(microsecond=0).isoformat() + "Z",
        open_positions=out,
        message=None if out else "Fichier présent mais vide.",
    )
